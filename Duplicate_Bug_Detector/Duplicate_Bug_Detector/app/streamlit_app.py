...
EXPECTED_APP_VERSION = "v7_live_github_token_ground_truth_eval"

st.set_page_config(page_title="Duplicate Bug Detector v7", layout="wide")
...
def decision(results, mixed_evidence):
    if results.empty:
        return "NO STRONG DUPLICATE FOUND - okay to continue", "success"

    top_score = float(results.loc[0, "duplicate_score"])

    if top_score >= thresholds["likely_duplicate"]:
        return "LIKELY DUPLICATE - review before saving", "error"

    if mixed_evidence:
        return "MIXED EVIDENCE - title and description match different existing issues; review manually", "warning"

    if top_score >= thresholds["possible_duplicate"]:
        return "POSSIBLE DUPLICATE - review suggested", "warning"
...
def search(repo_name, title, body, top_k=10):
    ...
    rows = []
    for pos in np.argsort(duplicate_scores)[::-1]:
...
if st.button("Check before save", type="primary"):
    ...
    top_score = float(results.loc[0, "duplicate_score"]) if not results.empty else 0.0
    st.metric("Top duplicate-risk score", f"{top_score:.3f}")

    if mixed_evidence:
        st.info(
            "The title and description appear to point to different existing issues. "
            f"Best title match: {mixed_details.get('title_issue')} ({mixed_details.get('title_score', 0):.3f}). "
            f"Best description match: {mixed_details.get('body_issue')} ({mixed_details.get('body_score', 0):.3f})."
        )

    st.caption(
        "Decision uses duplicate_score. This score combines semantic meaning, bug-description similarity, "
        "title similarity, technical-word similarity, and error/text-fragment similarity. "
        "A high-confidence duplicate requires coherent evidence for the same candidate issue."
    )
